          ...
          {!ads?.length ? (
            <div className="text-[#2B3467] text-sm p-3  md:text-2xl md:p-5 h-full">
              <div className="h-96 my-10 flex items-center">
                <h2>Hey <span className="text-[#EB455F]">{auth?.user.name ? auth?.user.name : auth?.user.username}</span>, You have not liked any properties yet!</h2>
              </div>
            </div>
          ) : (
            <div className="container">
              <div className="text-3xl p-5 mt-12">
                <p className="text-[#2B3467]">You have liked <span className="text-[#EB455F]">{ads.length}</span> properties.</p>
              </div>
